...
function SetSymbol() {
    ...
    useEffect(()=>{     
        fetch(`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${search}&apikey=Z4O3UPNJDXBU125A`)
    ...
    const addStockToBlockChain = async (stock) => {
        if(stock){
        fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${stock["1. symbol"]}&apikey=Z4O3UPNJDXBU125A`)
        .then(res => res.json()).then((data)=>{
